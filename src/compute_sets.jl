...
function compute_sets(data::Dict{String})
    return (;
        ...
        RCIE_P = Dict{Tuple{String,String,String},Vector{String}}(
            ...
            g in DF.groupby(data["RPC_IRE"], [:ALL_REG, :C, :IE])
        ),
        RP_ACE = if isempty(data["RPC_ACE"])
            nothing
        else
            Dict{Tuple{String,String},Vector{String}}(
                (g.REG[1], g.PRC[1]) => g.CG for g in DF.groupby(data["RPC_ACE"], [:REG, :PRC])
            )
        end,
        R_P = Dict{String,Vector{String}}(g.R[1] => g.P for g in DF.groupby(data["RP"], :R)),
        R_C = Dict{String,Vector{String}}(g.R[1] => g.C for g in DF.groupby(data["RC"], :R)),
        RP_C = Dict{Tuple{String,String},Vector{String}}(
            (g.R[1], g.P[1]) => g.C for g in DF.groupby(data["RPC"], [:R, :P])
        ),
        R_CPT = Dict{String,Vector{Tuple{Int16,Int16,String}}}(
            g.R[1] => Tuple.(eachrow(g[!, [:ALLYEAR, :T, :PRC]])) for
            g in DF.groupby(data["RTP_CPTYR"], :R)
        ),
    )
end
